...
exports.saveFaq = (req, res) => {
  Faq.find((err, result) => {
    ...
    if (result.length > 0) {
      let filter = { _id: result[0]._id };
      let update = { content: req.body.content };
      Faq.findOneAndUpdate(filter, update, (error, result1) => {
        if (error) {
  ...
  exports.saveTerms = (req, res) => {
    Terms.find((err, result) => {
      ...
      if (result.length > 0) {
        let filter = { _id: result[0]._id };
        let update = { content: req.body.content };
        Terms.findOneAndUpdate(filter, update, (error, result1) => {
          if (error) {
            res.json({ success: false, message: error.message });
          }
          res.json({ success: true });
